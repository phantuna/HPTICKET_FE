...
import { AlertTriangle, UploadCloud, Zap, HardDrive } from 'lucide-react';
import { BackupFile, systemService } from '../../../api/systemService';
import { toast } from '../../../shared/utils/toast';
import { ConfirmModal } from '../../../shared/components/ConfirmModal';
import ChunkedUploadRestore from './ChunkedUploadRestore';
...
const RestoreDatabase: React.FC<RestoreDatabaseProps> = ({ files, onRefresh }) => {
  ...
  const [uploadTab, setUploadTab] = useState<UploadTab>('simple');
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-1 flex gap-1">
        <button
          ...
          className={`flex-1 flex items-center justify-center gap-2 py-2.5 rounded-lg text-sm font-medium transition-all
            ${uploadTab === 'simple' ? 'bg-blue-500 text-white shadow-sm' : 'text-gray-500 hover:bg-gray-50'}`}
        >
          <Zap className="w-4 h-4" />
          Upload nhanh <span className="text-xs opacity-70">(dưới 100MB)</span>
        </button>
        <button
          onClick={() => setUploadTab('chunked')}
          className={`flex-1 flex items-center justify-center gap-2 py-2.5 rounded-lg text-sm font-medium transition-all
            ${uploadTab === 'chunked' ? 'bg-purple-500 text-white shadow-sm' : 'text-gray-500 hover:bg-gray-50'}`}
        >
          <HardDrive className="w-4 h-4" />
          Upload lớn <span className="text-xs opacity-70">(5GB+)</span>
        </button>
      </div>

      {/* Simple upload (giữ nguyên giao diện cũ) */}
      {uploadTab === 'simple' && (
      ...
      {uploadTab === 'chunked' && (
        <ChunkedUploadRestore onSuccess={onRefresh} />
      )}
    </div>
  );
};
